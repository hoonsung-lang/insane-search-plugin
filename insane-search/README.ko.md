...
- **단일 수집(content/trace 1회 반환):** `--json-content`로 메타데이터·trace와 비신뢰 콘텐츠 경계로 감싼 본문을 함께 받는다. 진단을 위해 같은 URL을 다시 수집하지 않아도 된다. 기존 `--json`은 본문을 제외하는 형식을 유지한다.
- **PDF 파서 지연 로딩:** `pdfplumber`·`pypdf`는 PDF 추출이 필요할 때만 로드하며, 일반 HTML 엔진 시작에는 로드하지 않는다. PDF 추출과 폴백 동작은 유지한다.
