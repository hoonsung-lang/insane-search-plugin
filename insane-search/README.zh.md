...
- **一次抓取返回正文与 trace：** `--json-content` 同时返回元数据、trace 和带有不可信内容边界标记的正文，无需为诊断再次抓取同一 URL。原有 `--json` 输出仍不包含正文。
- **PDF 解析器延迟加载：** 仅在需要提取 PDF 时加载 `pdfplumber` 和 `pypdf`，普通 HTML 引擎启动时不加载。PDF 提取与回退行为保持不变。
